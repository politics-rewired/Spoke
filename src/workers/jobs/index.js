...
export async function assignTexters(job) {
  ...
  const campaign = await r.reader("campaign").where({ id: cid }).first();
  ...
  await r.knex.transaction(async (trx) => {
    ...
    await trx.raw(
      `
        ...
        where
          ...
          ${assignmentIds > 0 ? "and assignment_id <> ANY(?)" : ""}
      `,
      assignmentIds.length > 0 ? [cid, assignmentIds] : [cid]
    );
    ...
    for (const assignmentTarget of assignmentTargets) {
      ...
      await trx.raw(
        ...
        `,
        [cid, assignmentId, contactsCount, cid, assignmentId]
      );
    }
      ...
      await trx.raw(
        `
          ...
          where
            ...
            and archived = ${campaign.is_archived}
        `,
        [cid, assignmentId, contactsCount, assignmentId]
      );
    }
