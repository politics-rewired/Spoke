...
async function go() {
  ...
  const contactsToReassign = await getNCampaignContactsToReassignFromCampaign(amount_to_reassign, campaign_id)
  const messagesMatchingContacts = await getMessagesForContacts(contactsToReassign)
...
async function getNCampaignContactsToReassignFromCampaign(n, campaign_id) {
  return await db('campaign_contact').where({ 
...
async function getMessagesForContacts(campaign_contacts) {
  const messageArrays = await Promise.all(campaign_contacts.map(getMessagesForContact))
  let result = []
...
async function lookupByName(name) {
  ...
  const possibleMatches = await db('user').where({ first_name, last_name }).select('id');
  if (possibleMatches.length > 0) {
    return possibleMatches[0].id
  } else {
    throw new Error(`Not sure who ${JSON.stringify({first_name, last_name})} is.`)
  }
}
...
async function getNCampaignContactsToReassign(n) {
  ...
  currentCampaign.campaign_contact_ids = currentCampaign.campaign_contact_ids.slice(n, BIG_NUMBER)
  const result = [ { campaign_id: currentCampaign.id, campaign_contact_ids: bunch }  ]
  if (bunch.length == n) {
    return result;
  } else {
...
async function ensureAssignmentId(user_id, campaign_id) {
  const existingAssignments = await db('assignment').select('*').where({ user_id, campaign_id })
  const existingAssignment = existingAssignments[0]
  if (!existingAssignment) {
    const assignment = await db('assignment').insert({ 
      user_id,
      campaign_id
    }).returning('id')
