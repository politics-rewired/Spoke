...
async function associateMessage(message) {
  ...
  const update = await db('message').where({ id: message.id }).update({ campaign_contact_id: matchingCampaignContact.id })
  return update;
}

async function findMatchingCampaignContact(message) {
  // console.log(`\n\nDoing ${message.id}\n\n`)
  const campaign_contacts = await db('campaign_contact').where({ cell: message.contact_number })
  const with_same_assignment_id = campaign_contacts.filter(cc => cc.assignment_id == message.assignment_id)

  let selection
  try {
    selection = await selectCampaignContact(message, with_same_assignment_id, 'SAME ASSIGNMENT')
  } catch (ex) {
    // console.log('Looking for other assignment ids')
    selection = await selectCampaignContact(message, campaign_contacts, 'ALL ASSIGNMENTS')
  }
...
async function selectCampaignContact(message, options, mode) {
  if (options.length == 1) {
    ...
    if (created_before_message.length == 1) {
      ...
      throw new Error(`Could not associate message with campaign contact in mode ${mode}: ${prettyPrint(message)}.
