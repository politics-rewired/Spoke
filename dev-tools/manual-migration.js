...
async function copyAll(source, target, table) {
  const greatestTarget = await target(table).select('id').orderBy('id', 'desc').limit(1)
  const greatestTargetId = greatestTarget[0] ? greatestTarget[0].id : 0

  console.log(`[${table}]: Syncing since `, greatestTargetId)

  const countToTransfer = Object.values((await source(table).count('id').where('id', '>', greatestTargetId))[0])[0]
  ...
  const allBatches = new Array(batchCount).fill(null).map((_i, n) => ({offset: (n) * INSERT_BATCH_SIZE, limit: INSERT_BATCH_SIZE, batch: n}))
  ...
  return new Promise((resolve, reject) => {
    ...
    function queueBatch() {
      ...
      allPromises.push(
        copyBetween(source, target, table, greatestTargetId, batch.offset, batch.limit).then(rowsCopied => {
          console.log(`[${table}] Did ${batch.batch} / ${batchCount}:`, (lastBatchQueued) * INSERT_BATCH_SIZE)
    ...
    console.log(`[${table}] Initial row concurrency is ${initialRowConcurrency}`)
    let queuedSoFar = 0
...
async function copyBetween(source, target, table, greatestTargetId, offset, limit) {
  let rowsToCopy = []

  async function fetchNextBatch() {
    rowsToCopy = await source(table).select('*').where('id', '>', greatestTargetId + offset).where('id', '<=', greatestTargetId + offset + limit)
