const config = {
  ...
  // async function getNDuplicates(n, fromCampaigns) {
  //   const whereClause = fromCampaigns ? `WHERE campaign_contact.campaign_id IN (${fromCampaigns.join(', ')})`
  ...
  //     const deleteResult = await db('campaign_contact').whereIn('id', updatedToDelete).del()
  //   } catch (error) {
  //     const failedId = error.sql.split(' ').reverse()[0].split('(')[1].split(')')[0]
  ...
  // let done = 1;
  // async function main() {
  //   let listOfDuplicates = await getNDuplicates(BATCH_SIZE, [66])
  //   // console.log(listOfDuplicates)
  //   // for (let phone of Object.keys(listOfDuplicates)) {
  
  //   await Promise.all(Object.keys(listOfDuplicates).map(async phone => {
  //     const duplicates = listOfDuplicates[phone]
  //     const hasReceivedMessage = duplicates.filter(cc => cc.message_status !== 'needsMessage').length > 0;
  //     // console.log({ hasReceivedMessage })
  //     if (hasReceivedMessage) {
  //       const toDelete = duplicates.filter(cc => cc.message_status == 'needsMessage')
